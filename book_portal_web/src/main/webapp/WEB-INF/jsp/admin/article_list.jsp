...
<table class="table table-sm">
    ...
    <tr>
        ...
        <td>${article.click}</td>
        <td><button type="button" class="btn btn-outline-info btn-sm" onclick="detailScreen('《${article.title}》','/admin/article/detail?id=${article.id}')">详情</button></td>
        <td><button type="button" class="btn btn-outline-success btn-sm" onclick="commentScreen('《${article.title}》|评论管理','/admin/article/comment?id=${article.id}')">评论</button></td>
        <td><button type="button" class="btn btn-outline-primary btn-sm" onclick="editScreen('《${article.title}》|编辑','/admin/article/edit?id=${article.id}')">编辑</button>&nbsp;&nbsp;</td>
...
<nav aria-label="Page navigation example" style="position: absolute;bottom: 10px;left: 42%">
    <ul class="pagination justify-content-center">
        ...
        <li class="page-item  <c:if test="${pageInfo.pageNum==pageInfo.total}">disabled</c:if> ">
            <a class="page-link" href="/admin/article/list?page=${pageInfo.total}">&raquo;</a>
        </li>
    </ul>
</nav>
